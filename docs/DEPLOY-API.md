...
The app does not try to pass the check itself. Stop Cloudflare from
challenging the API instead: it is JSON for the apps, not pages for people.
...
- **Hostinger CDN** (if Hostinger manages the domain's Cloudflare):
